...
<u:page title="${title}">
	...
	<c:if test="${not empty task and not empty task.square and not empty task.circumCircle}">
		<h3>Решение задачи</h3>
		<p>Квадрат с вершинами в точках: <span class="math"><i>P</i><sub>1</sub>(<span id="square_vertex_1_x">${task.square.vertex1.x}</span>,&nbsp;<span id="square_vertex_1_y">${task.square.vertex1.y}</span>)</span>, <span class="math"><i>P</i><sub>2</sub>(<span id="square_vertex_2_x">${task.square.vertex2.x}</span>,&nbsp;<span id="square_vertex_2_y">${task.square.vertex2.y}</span>)</span>, <span class="math"><i>P</i><sub>3</sub>(<span id="square_vertex_3_x">${task.square.vertex3.x}</span>,&nbsp;<span id="square_vertex_3_y">${task.square.vertex3.y}</span>)</span>, <span class="math"><i>P</i><sub>4</sub>(<span id="square_vertex_4_x">${task.square.vertex4.x}</span>,&nbsp;<span id="square_vertex_4_y">${task.square.vertex4.y}</span>)</span></p>
		<p>Описанная вокруг этого квадрата окружность с центром в точке <span class="math"><i>C</i>(<span id="circle_center_x">${task.circumCircle.center.x}</span>,&nbsp;<span id="circle_center_y">${task.circumCircle.center.x}</span>)</span> и радиусом <span class="math"><i>R</i>&nbsp;=&nbsp;<span id="circle_radius">${task.circumCircle.radius}</span></span></p>
	</c:if>
</u:page>